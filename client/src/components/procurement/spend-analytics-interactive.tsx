...
export default function SpendAnalyticsDashboard({ className }: SpendAnalyticsDashboardProps) {
  ...
  const resolveAnomaly = (id: number) => {
    if (resolvedAnomalies.includes(id)) {
      setResolvedAnomalies(resolvedAnomalies.filter(a => a !== id));
    } else {
      setResolvedAnomalies([...resolvedAnomalies, id]);
    }
  };
  
  const implementForecast = (id: number) => {
    if (implementedForecasts.includes(id)) {
      setImplementedForecasts(implementedForecasts.filter(f => f !== id));
    } else {
      setImplementedForecasts([...implementedForecasts, id]);
    }
  };
          ...
          {activeTab === "category" && (
            <div className="space-y-6">
              ...
              <div className="space-y-4 px-1">
                {categoryData
                  .filter(category => {
                    ...
                    if (categoryFilter === 'increasing') return category.trend === 'up';
                    if (categoryFilter === 'decreasing') return category.trend === 'down';
                    if (categoryFilter === 'stable') return category.trend === 'stable';
                    if (categoryFilter === 'anomaly') return category.anomaly;
                    return true;
                  })
                  .map((category, index) => (
              ...
              <div className="text-xs text-muted-foreground text-center mt-2">
                Showing {categoryData.filter(category => {
                  if (categoryFilter === 'all') return true;
                  if (categoryFilter === 'increasing') return category.trend === 'up';
                  if (categoryFilter === 'decreasing') return category.trend === 'down';
                  if (categoryFilter === 'stable') return category.trend === 'stable';
                  if (categoryFilter === 'anomaly') return category.anomaly;
                  return true;
                }).length} of {categoryData.length} categories • Last updated 4 minutes ago
              </div>
            </div>
          )}
                        ...
                        <div className="flex items-start justify-between">
                          ...
                          <div>
                            <Button 
                              ...
                              className={`h-7 text-xs ${
                                resolvedAnomalies.includes(anomaly.id) 
                                  ? 'border-green-200 text-green-600 hover:text-green-700 hover:bg-green-100 bg-green-50' 
                                  : 'border-red-200 text-red-600 hover:text-red-700 hover:bg-red-100 bg-red-50'
                              }`}
                        ...
                        <div className="flex items-start justify-between">
                          <div>
                            ...
                            <p className="mt-2 text-xs flex items-center">
                              ...
                              <span className={implementedForecasts.includes(forecast.id) 
                                ? 'text-green-600' 
                                : 'text-blue-600'
                              }>
                                {forecast.impact} 
                          ...
                          <div>
                            <Button 
                              ...
                              className={`h-7 text-xs ${
                                implementedForecasts.includes(forecast.id) 
                                  ? 'border-green-200 text-green-600 hover:text-green-700 hover:bg-green-100 bg-green-50' 
                                  : 'border-blue-200 text-blue-600 hover:text-blue-700 hover:bg-blue-100 bg-blue-50'
                              }`}
                              onClick={() => implementForecast(forecast.id)}
                            >
                              {implementedForecasts.includes(forecast.id) ? (
                                <>Undo Implementation</>
                              ) : (
                                <><Check className="h-3 w-3 mr-1" /> Implement</>
                              )}
                            </Button>
                          </div>
                        </div>
